...
function ScenarioCard({ label, finance, returnRate }: { label: string; finance: FinanceInputs; returnRate: number }) {
...
function Trend({ snapshots }: { snapshots: { date: string; netWorth: number }[] }) {
  ...
  if (points.length < 2) {
    return <p className="text-sm text-muted-foreground">Save at least two ledger dates to see invested-asset movement here.</p>;
  }
  const max = Math.max(...points.map((item) => Math.max(0, item.netWorth)), 1);
  const change = points[points.length - 1]!.netWorth - points[0]!.netWorth;
  return (
    <div className="flex flex-col gap-4">
      <p className="text-sm">{change >= 0 ? "Up" : "Down"} {formatMoney(Math.abs(change))} across the saved dates shown.</p>
        ...
        {points.map((point) => {
          ...
          return (
            <div key={point.date} className="flex min-w-0 flex-1 flex-col items-center justify-end gap-1">
              <span className="text-[10px] tabular-nums text-muted-foreground">{formatMoney(point.netWorth)}</span>
              <div className="w-full rounded-t-md bg-steward/70" style={{ height: `${height}%` }} title={`${point.date}: ${formatMoney(point.netWorth)}`} />
...
export function StewardInsights() {
  ...
  const balanceSheet = balanceSheetPosition(finance);
  const conservative = Math.max(0, finance.expectedReturn - 3);
  const optimistic = Math.min(10, finance.expectedReturn + 2);

  const saveNumber = (key: "cash" | "emergencyReserve" | "debt" | "estimatedTaxRate", raw: string) => {
    const value = key === "estimatedTaxRate" ? Math.min(60, numberValue(raw)) : numberValue(raw);
    setState((previous) => ({ ...previous, finance: { ...previous.finance, [key]: value } }));
  };
    ...
    <div className="flex flex-col gap-4">
      <Card className="border-steward/30 bg-steward/5">
        ...
        <CardContent className="grid gap-4 pt-5 md:grid-cols-3">
          <div>
            <p className="text-xs tracking-wide text-muted-foreground uppercase">Invested assets used by FI</p>
            <p className="font-heading text-2xl tabular-nums">{formatMoney(plan.fiCapital)}</p>
            <p className="text-xs text-muted-foreground">the existing sprint stays investment-based</p>
          </div>
          <div>
          ...
          <div>
            ...
            <p className="font-heading text-2xl tabular-nums">{Number.isFinite(sprint.grossIncomeLift) ? formatMoney(sprint.grossIncomeLift) : "—"}</p>
      ...
      <section className="grid gap-4 lg:grid-cols-2">
        <Card className="bg-card/80">
          ...
          <CardContent className="grid gap-4 pt-5 sm:grid-cols-2">
            <MoneyInput id="fi-cash" label="Cash" value={finance.cash} hint="Track liquid cash separately from invested assets." onSave={(raw) => saveNumber("cash", raw)} />
            <MoneyInput id="fi-reserve" label="Emergency reserve" value={finance.emergencyReserve} hint="Protected cash you do not want the sprint to consume." onSave={(raw) => saveNumber("emergencyReserve", raw)} />
            <MoneyInput id="fi-debt" label="Debt" value={finance.debt} hint="Visible here so a large portfolio cannot hide the balance-sheet obligation." onSave={(raw) => saveNumber("debt", raw)} />
            <div className="flex flex-col gap-2">
              <Label htmlFor="fi-tax-rate">Estimated tax on new income</Label>
              <div className="relative">
                <Input id="fi-tax-rate" inputMode="decimal" defaultValue={finance.estimatedTaxRate} onBlur={(event) => saveNumber("estimatedTaxRate", event.target.value)} className="pr-8" />
                <span className="pointer-events-none absolute top-1/2 right-3 -translate-y-1/2 text-sm text-muted-foreground">%</span>
              </div>
              <p className="text-xs leading-relaxed text-muted-foreground">Used only to translate the take-home gap into an approximate gross-income target.</p>
            </div>
            <div className="sm:col-span-2 rounded-xl bg-muted/60 p-4 text-sm leading-relaxed">
              Balance-sheet check after protecting the reserve: <strong>{formatMoney(balanceSheet)}</strong>. The FI sprint itself continues to use invested assets so the existing ledger and historical calculations remain comparable.
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="bg-card/80">
          ...
          <CardContent className="pt-5"><Trend snapshots={state.snapshots} /></CardContent>
        </Card>
      </section>
...
function MoneyInput({ id, label, value, hint, onSave }: { id: string; label: string; value: number; hint: string; onSave: (raw: string) => void }) {
  return (
    <div className="flex flex-col gap-2">
      <Label htmlFor={id}>{label}</Label>
      <div className="relative">
        <span className="pointer-events-none absolute top-1/2 left-2.5 -translate-y-1/2 text-sm text-muted-foreground">$</span>
        <Input id={id} inputMode="decimal" className="pl-6" defaultValue={value || ""} placeholder="0" onBlur={(event) => onSave(event.target.value)} />
      </div>
