...
const balanceKeys = new Set<MetricKey>(["netWorth", "cash", "debt"]);
...
function asDraft(preview: QuickenImportPreview): Draft {
  return Object.fromEntries(metricOrder.map((key) => {
    ...
    return [key, value == null ? "" : String(Math.round(value))];
  })) as Draft;
}

function asSelection(): Selection {
  return Object.fromEntries(metricOrder.map((key) => [key, false])) as Selection;
}
...
export function QuickenImport() {
  ...
  function currentValue(key: MetricKey) {
    if (key === "monthlyIncome") return state.finance.monthlyIncome;
    if (key === "netWorth") return state.finance.netWorth;
    return state.finance[key];
  }

  function toggleMetric(key: MetricKey, checked: boolean) {
    if (!draft) return;
    const available = preview ? previewValue(preview, key) != null : false;
    if (checked && !available && balanceKeys.has(key) && !draft[key]) {
      setDraft((previous) => previous ? { ...previous, [key]: String(Math.round(currentValue(key))) } : previous);
    }
    setSelected((previous) => previous ? { ...previous, [key]: checked } : previous);
  }
  ...
  function applyImport() {
    ...
    setMessage(`Updated ${changed} reviewed ${changed === 1 ? "value" : "values"} from ${fileName}.`);
    setPreview(null);
    setDraft(null);
    setSelected(null);
  }
      ...
      transactionDetails: includeTransactionDetails
        ? preview.transactionAudit
            ...
            .map(({ date, amount, payee, category, account, classification, confidence, includedInAverage, reason }) => ({
              date, amount, payee: payee.slice(0, 120), category: category.slice(0, 120), account: account.slice(0, 120), classification, confidence, includedInAverage, reason: reason.slice(0, 180),
            }))
        : "Not shared",
    };
    try {
        ...
        {preview && draft && selected ? (
          <div className="flex flex-col gap-5 rounded-2xl border border-border/80 bg-background/60 p-4 sm:p-5">
            ...
            <div className="grid gap-3">
              {metricOrder.map((key) => {
                const proposed = previewValue(preview, key);
                const available = proposed != null;
                const manualFallback = !available && balanceKeys.has(key);
                const current = currentValue(key);
                const enabled = available || manualFallback;
                return (
                  <label key={key} className={`grid gap-3 rounded-xl border p-3 sm:grid-cols-[auto_1fr_10rem] sm:items-center ${enabled ? "border-border" : "border-border/50 opacity-60"}`}>
                    <input type="checkbox" checked={selected[key]} disabled={!enabled} onChange={(event) => toggleMetric(key, event.target.checked)} className="size-4" />
                    <span><span className="block text-sm font-medium">{labels[key].title}</span><span className="block text-xs leading-relaxed text-muted-foreground">{labels[key].detail} Current: {formatMoney(current)}. {available ? <>Quicken proposes {formatMoney(proposed)}. Check the box to approve replacing the current value.</> : manualFallback ? <>This QIF does not contain a trustworthy current balance for this category. Check the box to enter and explicitly approve a replacement; Two Goals will not invent one from transaction history.</> : <>Quicken found no usable replacement value; the current value will remain unchanged.</>}</span></span>
                    <div className="relative"><span className="pointer-events-none absolute top-1/2 left-2.5 -translate-y-1/2 text-sm text-muted-foreground">$</span><Input inputMode="decimal" className="pl-6" value={draft[key]} disabled={!selected[key]} placeholder={manualFallback ? String(Math.round(current)) : undefined} onChange={(event) => setDraft((previous) => previous ? { ...previous, [key]: event.target.value } : previous)} aria-label={`${labels[key].title} replacement value`} /></div>
                  </label>
                );
              })}
            </div>

            <button type="button" onClick={() => setShowAudit((value) => !value)} className="flex w-full items-center justify-between rounded-xl border border-border bg-card px-4 py-3 text-left text-sm font-medium hover:bg-muted/50"><span>{showAudit ? "Hide" : "Review"} import details</span><ChevronDown className={`size-4 transition-transform ${showAudit ? "rotate-180" : ""}`} /></button>
            {showAudit ? <AuditDetails preview={preview} /> : null}

            <section className="rounded-2xl border border-faith/25 bg-faith/5 p-4"><div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div><p className="flex items-center gap-2 text-xs tracking-[0.16em] text-faith uppercase"><Bot className="size-4" /> AI audit</p><p className="mt-1 text-sm font-medium">Ask Guide to challenge the import before you trust it.</p><p className="mt-1 text-xs leading-relaxed text-muted-foreground">Account classifications, monthly totals, coverage, and warnings can be sent. Raw file contents are never uploaded.</p></div><button type="button" onClick={() => void askGuideToAudit()} disabled={guideLoading} className="inline-flex h-10 shrink-0 items-center justify-center rounded-lg bg-faith px-4 text-sm font-medium text-white disabled:opacity-50">{guideLoading ? "Reviewing…" : "Ask Guide to audit"}</button></div><label className="mt-3 flex items-start gap-2 text-xs text-muted-foreground"><input type="checkbox" checked={includeTransactionDetails} onChange={(event) => setIncludeTransactionDetails(event.target.checked)} className="mt-0.5 size-4" />Include up to 25 highest-risk transaction rows in this AI review. Off by default. Payee/category details may be sensitive.</label>{guideError ? <p role="alert" className="mt-3 rounded-lg border border-destructive/25 bg-destructive/5 p-3 text-sm text-destructive">{guideError}</p> : null}{guideAnswer ? <div className="mt-3 whitespace-pre-wrap rounded-xl border border-border bg-background p-4 text-sm leading-7">{guideAnswer}</div> : null}</section>

            {preview.warnings.length ? <div className="rounded-xl border border-border/80 bg-muted/40 p-3 text-xs leading-relaxed text-muted-foreground">{preview.warnings.map((warning) => <p key={warning}>{warning}</p>)}</div> : null}
            <div className="flex flex-wrap gap-2"><button type="button" onClick={applyImport} disabled={!metricOrder.some((key) => selected[key])} className="inline-flex h-10 items-center justify-center rounded-lg bg-steward px-4 text-sm font-medium text-white hover:bg-steward/90 disabled:cursor-not-allowed disabled:opacity-50">Update Two Goals</button><button type="button" onClick={() => { setPreview(null); setDraft(null); setSelected(null); setMessage(null); }} className="inline-flex h-10 items-center justify-center rounded-lg border border-border bg-background px-4 text-sm font-medium hover:bg-muted">Cancel</button></div>
          </div>
        ) : null}
...
function AuditDetails({ preview }: { preview: QuickenImportPreview }) {
  return <div className="flex flex-col gap-5">
    <section><h3 className="text-sm font-semibold">Accounts</h3><div className="mt-2 overflow-x-auto rounded-xl border border-border/80"><table className="w-full min-w-[42rem] text-xs"><thead className="bg-muted/50 text-left text-muted-foreground"><tr><th className="p-3">Account</th><th className="p-3">Type</th><th className="p-3">Balance</th><th className="p-3">Classified as</th><th className="p-3">Confidence</th><th className="p-3">Why</th></tr></thead><tbody>{preview.accountAudit.map((item, index) => <tr key={`${item.name}-${index}`} className="border-t border-border/60"><td className="p-3 font-medium">{item.name}</td><td className="p-3">{item.type || "—"}</td><td className="p-3 tabular-nums">{item.balance == null ? "—" : formatMoney(item.balance)}</td><td className="p-3">{item.classification}</td><td className="p-3">{item.confidence}</td><td className="p-3 text-muted-foreground">{item.reason}</td></tr>)}</tbody></table></div></section>
    <section><h3 className="text-sm font-semibold">Monthly calculation</h3><div className="mt-2 overflow-x-auto rounded-xl border border-border/80"><table className="w-full min-w-[34rem] text-xs"><thead className="bg-muted/50 text-left text-muted-foreground"><tr><th className="p-3">Month</th><th className="p-3">Income</th><th className="p-3">Living</th><th className="p-3">Giving</th><th className="p-3">Transfers excluded</th><th className="p-3">Rows used</th></tr></thead><tbody>{preview.monthlyAudit.map((row) => <tr key={row.month} className="border-t border-border/60"><td className="p-3 font-medium">{row.month}</td><td className="p-3 tabular-nums">{formatMoney(row.income)}</td><td className="p-3 tabular-nums">{formatMoney(row.living)}</td><td className="p-3 tabular-nums">{formatMoney(row.giving)}</td><td className="p-3 tabular-nums">{formatMoney(row.transfers)}</td><td className="p-3">{row.includedTransactions}</td></tr>)}</tbody></table></div></section>
    <section><div className="flex items-baseline justify-between gap-3"><h3 className="text-sm font-semibold">Transactions</h3><p className="text-xs text-muted-foreground">Showing up to 200 rows</p></div><div className="mt-2 max-h-[32rem] overflow-auto rounded-xl border border-border/80"><table className="w-full min-w-[58rem] text-xs"><thead className="sticky top-0 bg-muted text-left text-muted-foreground"><tr><th className="p-3">Date</th><th className="p-3">Payee</th><th className="p-3">Category</th><th className="p-3">Amount</th><th className="p-3">Class</th><th className="p-3">Used?</th><th className="p-3">Confidence</th></tr></thead><tbody>{preview.transactionAudit.slice(0, 200).map((item, index) => <tr key={`${item.date}-${item.payee}-${index}`} className={`border-t border-border/60 ${item.confidence === "low" ? "bg-faith/5" : ""}`}><td className="p-3">{item.date || "Invalid"}</td><td className="p-3 font-medium">{item.payee || "—"}</td><td className="p-3">{item.category || "—"}</td><td className="p-3 tabular-nums">{formatMoney(item.amount)}</td><td className="p-3">{item.classification}</td><td className="p-3">{item.includedInAverage ? "Yes" : "No"}</td><td className="p-3">{item.confidence}</td></tr>)}</tbody></table></div></section>
  </div>;
}
