                  ...
                  <label key={key} className={`grid gap-3 rounded-xl border p-3 sm:grid-cols-[auto_1fr_10rem] sm:items-center ${available ? "border-border" : "border-border/50 opacity-60"}`}>
                    ...
                    <span>
                      ...
                      <span className="block text-xs leading-relaxed text-muted-foreground">
                        {labels[key].detail} Current: {formatMoney(current)}. {available ? <>Quicken proposes {formatMoney(proposed ?? 0)}. Check the box to approve replacing the current value.</> : <>Quicken found no usable replacement value; the current value will remain unchanged.</>}
                      </span>
                    </span>
                    <div className="relative">
                      <span className="pointer-events-none absolute top-1/2 left-2.5 -translate-y-1/2 text-sm text-muted-foreground">$</span>
                      <Input inputMode="decimal" className="pl-6" value={draft[key]} disabled={!available || !selected[key]} onChange={(event) => setDraft((previous) => previous ? { ...previous, [key]: event.target.value } : previous)} aria-label={`${labels[key].title} Quicken proposed value`} />
                    </div>
                  </label>
                );
              })}
